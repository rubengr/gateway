...
prepare_environment() {
	...
	elif [ "x${boot_drive}" = "x/dev/nfs" ] ; then
		source="/dev/nfs"
		destination="/dev/mmcblk1"
	else
...
_format_opt_ext4() {
 	empty_line
 	echo_broadcast "==> Formatting opt with mkfs.ext4 ${ext4_options} ${openmotics_partition} -L ${opt_label}"
 	generate_line 80
 	empty_line
 	LC_ALL=C mkfs.ext4 -F ${ext4_options} ${openmotics_partition} -L ${opt_label}
 	generate_line 80
 	echo_broadcast "==> Formatting rootfs: ${openmotics_partition} complete"
	flush_cache
}
...
_generate_fstab() {
	...
	echo "${openmotics_partition}  /opt  ext4  noatime,errors=remount-ro  0  2" >> ${tmp_rootfs_dir}/etc/fstab
	echo "debugfs  /sys/kernel/debug  debugfs  defaults  0  0" >> ${tmp_rootfs_dir}/etc/fstab
	echo "tmpfs	 /tmp	tmpfs	nodev,nosuid,size=20%	0  0" >> ${tmp_rootfs_dir}/etc/fstab
...
_copy_rootfs() {
  ...
  generate_line 40
  get_rsync_options
  rsync -aAx $rsync_options /* ${tmp_rootfs_dir} --exclude={/dev/*,/proc/*,/sys/*,/tmp/*,/run/*,/mnt/*,/media/*,/lost+found,/lib/modules/*,/uEnv.txt,/opt/*} || write_failure
  generate_line 40
  echo_broadcast "==> rsync: / -> ${tmp_opt_dir}"
  generate_line 40
  rsync -aAx $rsync_options /opt/openmotics ${tmp_opt_dir} || write_failure
  flush_cache
  generate_line 40
...
__EOF__
    ...
    conf_boot_startmb=${conf_boot_startmb:-"4"}
    conf_boot_endmb=${conf_boot_endmb:-"2400"}
    ...
    sfdisk_boot_startmb="${conf_boot_startmb}"
    sfdisk_boot_size_mb="${conf_boot_endmb}"
    sfdisk_opt_startmb=$(($sfdisk_boot_startmb + $sfdisk_boot_size_mb))
    ...
    if [ "x${test_sfdisk}" = "x" ] ; then
      ...
      sfdisk_boot_startmb="${sfdisk_boot_startmb}M"
      sfdisk_boot_size_mb="${conf_boot_endmb}M"
      sfdisk_opt_startmb="${sfdisk_opt_startmb}M"
    fi
    ...
    echo_broadcast "sfdisk: [sfdisk ${sfdisk_options} ${destination}]"
    echo_broadcast "sfdisk: [${sfdisk_boot_startmb},${sfdisk_boot_size_mb},${sfdisk_fstype},*]"
    echo_broadcast "sfdisk: [${sfdisk_opt_startmb},,${sfdisk_fstype},-]"
    echo_broadcast "==> Partitionning"
    generate_line 60
    LC_ALL=C sfdisk ${sfdisk_options} "${destination}" <<-__EOF__
${sfdisk_boot_startmb},${sfdisk_boot_size_mb},${sfdisk_fstype},*
${sfdisk_opt_startmb},,${sfdisk_fstype},-
__EOF__
    generate_line 60
    flush_cache
    empty_line
    ...
    openmotics_partition="${destination}p2"
  fi
...
_prepare_future_rootfs() {
	...
	tmp_opt_dir=${tmp_opt_dir:-"/tmp/rootfs/opt"}
	...
	mkdir -p ${tmp_opt_dir} || true
	echo_broadcast "==> Mounting ${openmotics_partition} to ${tmp_opt_dir}"
	mount ${openmotics_partition} ${tmp_opt_dir} -o async,noatime
	empty_line
	generate_line 80 '='
}
...
_teardown_future_rootfs() {
  ...
  generate_line 40
  empty_line
  echo_broadcast "==> Unmounting ${tmp_opt_dir}"
  echo_broadcast "==> Unmounting ${tmp_rootfs_dir}"
  flush_cache
  umount ${tmp_opt_dir} || umount -l ${tmp_opt_dir} || write_failure
...
prepare_drive() {
  ...
  if [ "${boot_partition}x" != "${rootfs_partition}x" ] ; then
    ...
    tmp_rootfs_dir="/tmp/rootfs"
    opt_label="openmotics"
    tmp_opt_dir="/tmp/rootfs/opt"
    _prepare_future_rootfs
    media_rootfs="1"
    _copy_rootfs
    #_teardown_future_rootfs
...
check_if_macdir_exists() {
  mac=`/sbin/ip link show eth0 | grep ether | awk '{print $2}' | tr -d ':'`
  if [ ! -d /opt/clients/$mac ]; then
     generate_line 80 '!'
     echo_broadcast "Client.tgz for this bone is missing, don't waste time installing ... stop right here"
     generate_line 80 '!'
     exit  # Don't waste time installing as the client.tgz for this bone is missing.
  fi
}