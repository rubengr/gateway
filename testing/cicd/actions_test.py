...
class ActionsTest(unittest.TestCase):
    ...
    def test_do_group_action_on_off(self):
        ...
        response_json = self.tools.api_testee('get_group_action_configurations', self.token)
        ...
        self.assertTrue(bool(config),
                        'The config should not be empty when returned from get group action configurations API. Got: {0}'.format(
                            response_json))
        configured_actions = []
        for one in config:
        ...
        for one in configured_actions:
            ...
            self.tools.api_testee('do_group_action?{0}'.format(url_params), self.token)
    ...
    def test_do_group_action_on_off_authorization(self):
        ...
        response_json = self.tools.api_testee('get_group_action_configurations', 'some_token', expected_failure=True)
        self.assertEqual(response_json, 'invalid_token',
                         'Should not be able to return group action configurations without a valid token. Got: {0}'.format(
                             response_json))

        response_json = self.tools.api_testee('get_output_status', 'some_token', expected_failure=True)
        self.assertEqual(response_json, 'invalid_token',
                         'The get_group_action_configurations API call should return \'invalid_token\' when called with an invalid token. Got: {0}'.format(
                             response_json))

        invalid_group_action_id = 9999
        url_params = urllib.urlencode({'group_action_id': invalid_group_action_id})
        response_json = self.tools.api_testee('do_group_action?{0}'.format(url_params), self.token,
                                              expected_failure=True)
        self.assertEqual(response_json.get('msg'),
                         'group_action_id not in [0, 160]: {0}'.format(invalid_group_action_id),
                         'Should return an error message when calling do group action API with an invalid group action ID. Got: {0}'.format(
                             response_json))
    ...
    def test_set_group_action_configuration(self):
        ...
        one_group_action_config = {'id': i,
                                   ...
                                   'name': 'Test' + str(i)}
        url_params = urllib.urlencode({'config': json.dumps(
            one_group_action_config)})  # Will turn on output 0 if it's off and turn it back off, reverse if it's on already.

        self.tools.api_testee('set_group_action_configuration?{0}'.format(url_params), self.token)

        url_params = urllib.urlencode({'id': i})
        response_json = self.tools.api_testee('get_group_action_configuration?{0}'.format(url_params), self.token)
        self.assertEqual(response_json.get('config'), one_group_action_config,
                         'The new config should be the same as the present group action config. Got: returned: {0} configured: {1}'.format(
                             response_json.get('config'), one_group_action_config))
        ...
        self.assertTrue(self.tools.check_if_event_is_captured(self.GROUP_ACTION_TARGET_ID, 1),
                        'Should return true after calling do_group_action API. Got: {0}'.format(
                            self.tools.input_status))

        self.assertTrue(self.tools.check_if_event_is_captured(self.GROUP_ACTION_TARGET_ID, 0),
                        'Should untoggled output 0 after a moment and must show input releases. Got: {0}'.format(
                            self.tools.input_status))

    @exception_handler
    def test_set_group_action_configuration_authorization(self):
        """ Testing the setting up of one group action auth verification. """
        response_json = self.tools.api_testee('set_group_action_configuration', 'some_token', expected_failure=True)
        self.assertEqual(response_json, 'invalid_token',
    ...
    def test_set_group_action_configurations_authorization(self):
        ...
        response_json = self.tools.api_testee('set_group_action_configurations', 'some_token', expected_failure=True)
        self.assertEqual(response_json, 'invalid_token')
    ...
    def test_set_startup_action_configuration(self):
        ...
        self.tools.api_testee('set_startup_action_configuration?{0}'.format(url_params), self.token)

        response_json = self.tools.api_testee('get_startup_action_configuration', self.token)
        self.assertEqual(response_json.get('config'), config,
                         'The new config should be the same as the present startup action config. Got{0}'.format(
                             response_json))
        ...
        self.tools.token = self.tools.get_new_token('openmotics', '123456')

    @exception_handler
    def test_set_startup_action_configuration_authorization(self):
        """ Testing the setting up of the startup action configuration. """
        response_json = self.tools.api_testee('set_startup_action_configuration', 'some_token', expected_failure=True)
        self.assertEqual(response_json, 'invalid_token')
    ...
    def test_do_basic_action(self):
        ...
        self.assertTrue(self.tools.check_if_event_is_captured(i, 1),
                        'Should have toggled the tester\'s input. Got {0}, expected output ID to toggle: {1}'.format(
                            self.tools.input_status, i))
        ...
        self.assertTrue(self.tools.check_if_event_is_captured(i, 0),
                        'Should have unpressed the tester\'s input. Got {0}, expected output ID to untoggle: {1}'.format(
                            self.tools.input_status, i))
        self.tools.api_testee('get_output_status', self.token)

    @exception_handler
    def test_do_basic_action_authorization(self):
        """ Testing do basic action API auth verification. """
        response_json = self.tools.api_testee('do_basic_action', 'some_token', expected_failure=True)
        self.assertEqual(response_json, 'invalid_token')

        url_params = urllib.urlencode({'action_type': 165, 'action_number': 46})
        response_json = self.tools.api_testee('do_basic_action?{0}'.format(url_params), 'some_token',
                                              expected_failure=True)
        self.assertEqual(response_json, 'invalid_token')
    ...
    def test_motion_sensor_timer_short(self):
        ...
        result = self.tools.check_if_event_is_captured(i, 0)

        self.assertTrue(result,
                        'Should have unpressed the tester\'s input. Got {0}, expected output ID to untoggle: {1}'.format(
                            result, i))
        self.assertTrue(152 > self.tools.input_record.get(str(i))["0"] - self.tools.input_record.get(str(i))["1"] > 148,
                        'Should toggle off after around 2m30s. Got: {0}'.format(
                            self.tools.input_record.get(str(i))["0"] - self.tools.input_record.get(str(i))["1"]))
    ...
    def test_get_input_configuration_authorization(self):
        ...
        response_json = self.tools.api_testee('get_input_configuration?{0}'.format(url_params), 'some_token',
                                              expected_failure=True)
        self.assertEqual(response_json, 'invalid_token',
                         'Expecting the response to be invalid_token. Got: {0}'.format(response_json))

        response_json = self.tools.api_testee('get_input_configurations', 'some_token', expected_failure=True)
        self.assertEqual(response_json, 'invalid_token',
                         'Expecting the response to be invalid_token. Got: {0}'.format(response_json))
    ...
    def test_execute_group_action(self):
        ...
        self.tools.api_testee('set_input_configuration?{0}'.format(url_params), token)
        self.webinterface.set_output(id=input_number, is_on=True)
        self.assertTrue(self.tools.check_if_event_is_captured(0, 1),
                        'Should execute a group action to toggle on output 0.')
    ...
    def test_toggles_output(self):
        ...
        self.assertTrue(self.tools.check_if_event_is_captured(output_to_toggle, 1),
                        'The Tester\'s input module should see a press after toggling the Testee\'s output. Got: {0}'.format(
                            self.tools.input_status))
        time.sleep(0.3)
        self.webinterface.set_output(id=output_to_toggle, is_on=False)
        self.assertTrue(self.tools.check_if_event_is_captured(output_to_toggle, 1),
                        'The Tester\'s input module should keep seeing a press after toggling the Testee\'s output. Got: {0}'.format(
                            self.tools.input_status))
        time.sleep(0.3)
        self.webinterface.set_output(id=output_to_toggle,
                                     is_on=True)  # Toggling the Tester's output, The Testee's input will see a press that executes an action that toggles an output, to confirm the output toggling, we can check the Tester's input module (all modules are cross configured).
        self.assertTrue(self.tools.check_if_event_is_captured(output_to_toggle, 0),
                        'The Tester\'s input module should see a release after toggling the Testee\'s output. Got: {0}'.format(
                            self.tools.input_status))
        time.sleep(0.3)
        self.webinterface.set_output(id=output_to_toggle, is_on=False)
        self.assertTrue(self.tools.check_if_event_is_captured(output_to_toggle, 0),
                        'The Tester\'s input module should keep seeing a release after toggling the Testee\'s output. Got: {0}'.format(
                            self.tools.input_status))
    ...
    def test_turn_output_on_off(self):
        ...
        self.webinterface.set_output(id=output_to_toggle, is_on=False)
        self.assertTrue(self.tools.check_if_event_is_captured(output_to_toggle, 1),
                        'Even if the Tester\'s output is off, the Testee\'s input should keep seeing a press. Got: {0}'.format(
                            self.tools.input_status))
    ...
    def test_toggles_all_outputs(self):
        ...
        for i in xrange(self.INPUT_COUNT):
            self.assertTrue(self.tools.check_if_event_is_captured(i, 1),
                            'The Tester\'s input module should keep seeing a press since its a toggle action toggling the Testee\'s output. Got: {0}'.format(
                                self.tools.input_status))
        ...
        for i in xrange(self.INPUT_COUNT):
            self.assertTrue(self.tools.check_if_event_is_captured(i, 0),
                            'The Tester\'s input module should keep see seeing releases after toggling the Testee\'s output. Got: {0}'.format(
                                self.tools.input_status))
    ...
    def test_motion_sensor_timer_7m30s(self):
        ...
        self.assertTrue(result,
                        'Should have unpressed the tester\'s input. Got {0}, expected output ID to untoggle: {1}'.format(
                            result, input_output_number))
        self.assertTrue(460 > self.tools.input_record.get(str(input_output_number))["0"] -
                        self.tools.input_record.get(str(input_output_number))["1"] > 440,
                        'Should toggle off after around 7m30s. Got: {0}'.format(
                            self.tools.input_record.get(str(input_output_number))["0"] -
                            self.tools.input_record.get(str(input_output_number))["1"]))
    ...
    def test_motion_sensor_timer_15m(self):
        ...
        self.assertTrue(result,
                        'Should have unpressed the tester\'s input. Got {0}, expected output ID to untoggle: {1}'.format(
                            result, input_output_number))
        self.assertTrue(910 > self.tools.input_record.get(str(input_output_number))["0"] -
                        self.tools.input_record.get(str(input_output_number))["1"] > 890,
                        'Should toggle off after around 15m. Got: {0}'.format(
                            self.tools.input_record.get(str(input_output_number))["0"] -
                            self.tools.input_record.get(str(input_output_number))["1"]))
    ...
    def test_motion_sensor_timer_25m(self):
        ...
        self.assertTrue(result,
                        'Should have unpressed the tester\'s input. Got {0}, expected output ID to untoggle: {1}'.format(
                            result, input_output_number))
        self.assertTrue(1510 > self.tools.input_record.get(str(input_output_number))["0"] -
                        self.tools.input_record.get(str(input_output_number))["1"] > 1490,
                        'Should toggle off after around 25m. Got: {0}'.format(
                            self.tools.input_record.get(str(input_output_number))["0"] -
                            self.tools.input_record.get(str(input_output_number))["1"]))
    ...
    def test_motion_sensor_timer_37m(self):
        ...
        self.assertTrue(result,
                        'Should have unpressed the tester\'s input. Got {0}, expected output ID to untoggle: {1}'.format(
                            result, input_output_number))
        self.assertTrue(2230 > self.tools.input_record.get(str(input_output_number))["0"] -
                        self.tools.input_record.get(str(input_output_number))["1"] > 2210,
                        'Should toggle off after around 37m. Got: {0}'.format(
                            self.tools.input_record.get(str(input_output_number))["0"] -
                            self.tools.input_record.get(str(input_output_number))["1"]))
    ...
    def test_motion_sensor_timer_52m(self):
        ...
        self.assertTrue(result,
                        'Should have unpressed the tester\'s input. Got {0}, expected output ID to untoggle: {1}'.format(
                            result, input_output_number))
        self.assertTrue(3130 > self.tools.input_record.get(str(input_output_number))["0"] -
                        self.tools.input_record.get(str(input_output_number))["1"] > 3110,
                        'Should toggle off after around 52m. Got: {0}'.format(
                            self.tools.input_record.get(str(input_output_number))["0"] -
                            self.tools.input_record.get(str(input_output_number))["1"]))
    ...
    def test_time_no_overrule_2m30s(self):
        ...
        self.assertTrue(result,
                        'Should have unpressed the tester\'s input. Got {0}, expected output ID to untoggle: {1}'.format(
                            result, input_output_number))
        self.assertTrue(158 > self.tools.input_record.get(str(input_output_number))["0"] -
                        self.tools.input_record.get(str(input_output_number))["1"] > 148,
                        'Should still turn off after 2m30s since it first turned on. Got: {0}'.format(
                            self.tools.input_record.get(str(input_output_number))["0"] -
                            self.tools.input_record.get(str(input_output_number))["1"]))
    ...
    def test_time_no_overrule_7m30s(self):
        ...
        self.assertTrue(460 > self.tools.input_record.get(str(input_output_number))["0"] -
                        self.tools.input_record.get(str(input_output_number))["1"] > 440,
                        'Should still turn off after 7m30s since it first turned on. Got: {0}'.format(
                            self.tools.input_record.get(str(input_output_number))["0"] -
                            self.tools.input_record.get(str(input_output_number))["1"]))
    ...
    def test_time_no_overrule_15m(self):
        ...
        self.assertTrue(910 > self.tools.input_record.get(str(input_output_number))["0"] -
                        self.tools.input_record.get(str(input_output_number))["1"] > 890,
                        'Should still turn off after 15m since it first turned on. Got: {0}'.format(
                            self.tools.input_record.get(str(input_output_number))["0"] -
                            self.tools.input_record.get(str(input_output_number))["1"]))
    ...
    def test_time_no_overrule_25(self):
        ...
        self.assertTrue(1510 > self.tools.input_record.get(str(input_output_number))["0"] -
                        self.tools.input_record.get(str(input_output_number))["1"] > 1490,
                        'Should still turn off after 25m since it first turned on. Got: {0}'.format(
                            self.tools.input_record.get(str(input_output_number))["0"] -
                            self.tools.input_record.get(str(input_output_number))["1"]))
    ...
    def test_time_no_overrule_37m(self):
        ...
        self.assertTrue(2230 > self.tools.input_record.get(str(input_output_number))["0"] -
                        self.tools.input_record.get(str(input_output_number))["1"] > 2210,
                        'Should still turn off after 37m since it first turned on. Got: {0}'.format(
                            self.tools.input_record.get(str(input_output_number))["0"] -
                            self.tools.input_record.get(str(input_output_number))["1"]))
    ...
    def test_time_no_overrule_52m(self):
        ...
        self.assertTrue(3130 > self.tools.input_record.get(str(input_output_number))["0"] -
                        self.tools.input_record.get(str(input_output_number))["1"] > 3110,
                        'Should still turn off after 52m since it first turned on. Got: {0}'.format(
                            self.tools.input_record.get(str(input_output_number))["0"] -
                            self.tools.input_record.get(str(input_output_number))["1"]))
    ...
    def test_do_basic_action_flash_led_output(self):
        ...
        input_number = randint(0, 7)  # Get a random input number
        url_params = urllib.urlencode(
            {'action_type': 65, 'action_number': input_number})  # ActionType 65 flashes output number X.
        token = self.tools.get_new_token('openmotics', '123456')
        response = self.tools.api_testee('do_basic_action?{0}'.format(url_params), token)
        self.assertTrue(response.get('success'),
                        'Should return true to indicate a successful API call. Got: {0}'.format(response))
    ...
    def test_do_basic_action_flash_led_input(self):
        ...
        input_number = randint(0, 7)  # Get a random input number
        url_params = urllib.urlencode(
            {'action_type': 66, 'action_number': input_number})  # ActionType 66 flashes input number X.
        token = self.tools.get_new_token('openmotics', '123456')
        response = self.tools.api_testee('do_basic_action?{0}'.format(url_params), token)
        self.assertTrue(response.get('success'),
                        'Should return true to indicate a successful API call. Got: {0}'.format(response))
    ...
    def test_thermostat_setpoint_action_type(self):
        ...
        token = self.tools.get_new_token('openmotics', '123456')
        self.tools.api_testee('do_basic_action?{0}'.format(url_params), token)
        response = self.tools.api_testee('get_thermostat_status', token)
        self.assertEqual(response.get('status')[0].get('csetp'), 16,
    ...
    def test_thermostat_increase_setpoint_action_type(self):
        ...
        response = self.tools.api_testee('get_thermostat_status', self.token)
        self.assertEqual(response.get('status')[0].get('csetp'), 16.5,
    ...
    def test_thermostat_decrease_setpoint_action_type(self):
        ...
        response = self.tools.api_testee('get_thermostat_status', self.token)
        self.assertEqual(response.get('status')[0].get('csetp'), 22,
    ...
    def test_turn_only_lights_off(self):
        ...
        for i in xrange(8):
            ...
            url_params = urllib.urlencode({'config': json.dumps(config)})
            self.tools.api_testee('set_output_configuration?{0}'.format(url_params), self.token)

        url_params = urllib.urlencode(
            {'action_type': 163, 'action_number': randint(0, 239)})  # ActionType 172 will turn all lights by floor.
        self.tools.api_testee('do_basic_action?{0}'.format(url_params), self.token)
        ...
        url_params = urllib.urlencode({'action_type': 173, 'action_number': 3})  # Turning all outputs off.
        self.tools.api_testee('do_basic_action?{0}'.format(url_params), self.token)
    ...
    def _set_group_actions_config(self):
        ...
        return self.tools.api_testee('set_group_action_configurations?{0}'.format(url_params), token), config
    ...
    def _set_input_room_config(self):
        ...
        return self.tools.api_testee('set_room_configurations?{0}'.format(url_params), self.token)
    ...
    def _set_default_output_config(self):
        for i in xrange(8):
            ...
            token = self.tools.get_new_token('openmotics', '123456')
            url_params = urllib.urlencode({'config': json.dumps(config)})
            self.tools.api_testee('set_output_configuration?{0}'.format(url_params), token)
